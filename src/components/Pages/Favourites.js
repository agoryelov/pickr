...
class Favourites extends React.Component {
  ...
  createList2 = () => {
    ...
    const saved = this.state.list;
    return (
      <div style={{ padding: '2em' }}>
        {saved.map(x =>
          <div key={x['questID']}>
            <SavedQuestItem questData={data[x['questID']]} />
          </div>
        )}
      </div>
    );
  ...
  render() {
    ...
    if (!this.state.loading) {
      ...
      content = <div><div className="favourites">{this.createList2()} </div> <div className="completed">Completed Quests {this.createCompletedList2()}</div></div>
    } else {
      content =
        <div>
        </div>;
